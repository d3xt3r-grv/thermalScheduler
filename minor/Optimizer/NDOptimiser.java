...
public class NDOptimiser {
    ...
    public NDOptimiser(int maxIterations, int numTasks, int searchAgents, int numObj, int maxArchiveSize, int nVm){
    ...
    public List<Integer> randomSolution(){
        List<Integer> temp = new ArrayList<>();
    ...
    public void startOptimisation(Runner runner) throws IOException {
        ...
        ScatterPlot plotComparison= new ScatterPlot("Non Domination Sorting iterative development");
        ScatterPlot results = new ScatterPlot("Non Domination Sorting Optimiser results");
        ...
        for(currIter=0;currIter<maxIterations;currIter++){
            ...
            for(int i = 0; i< searchAgents; i++){
                antPosition.add(randomSolution());
            }
        }
