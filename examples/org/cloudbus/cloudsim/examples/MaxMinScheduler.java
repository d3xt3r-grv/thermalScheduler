...
public class MaxMinScheduler extends BaseCloudletScheduler{
...
public void run(){
		...
		try{
			File maxminfile = new File("MaxMinResults");
			BufferedWriter maxminWriter = new BufferedWriter(new FileWriter(maxminfile));
		Log.printLine("MAX MIN scheduler running with " + Cloudletlist.size()
		        + " cloudlets.");
		...
		adjustforfailures();
        System.out.println("\nAfter adjusting MAX MIN schedule for failures");
		...
		while(true){
			...
			for(Object cloudletObject:Cloudletlist){
				...
				if(chkavail && !chkassign){
					...
					//select cloudlet with maximum completion time and the corresponding minimum vm
			...
			if(maxCloudlet != null){
				allocateminCloudlet(maxCloudlet,minVmfinal, minreadyTime, mincompletionTime);
			}
